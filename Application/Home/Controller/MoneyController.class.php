...
class MoneyController extends HomeController
{
    ...
    public function index()
    {
        if (IS_POST) {
            ...
            if ($num < $money_min) {
                $this->error('Conduct financial transactions Quantity exceed system Least limit 10!');
            }
            ...
            $mo->execute('lock tables codono_user_coin write  , codono_money_log  write , codono_money  write');
            $rs = array();
            $rs[] = $mo->table('codono_user_coin')->where(array('userid' => userid()))->setDec($money['coinname'], $num);
            $rs[] = $mo->table('codono_money_log')->add(array('userid' => userid(), 'moneyid' => $money['id'], 'name' => $money['name'], 'num' => $num, 'tian' => $money['tian'], 'fee' => $money['fee'], 'feecoin' => $money['feecoin'], 'addtime' => time(), 'status' => 0));
            ...
            if ($money['endtime'] < $money['lasttime']) {
                ...
                if ($MoneyLogList) {
                    ...
                    foreach ($MoneyLogList as $user_money_list) {
                        ...
                        $mo->execute('set autocommit=0');
                        $mo->execute('lock tables codono_user_coin write,codono_money_log  write,codono_money_dlog  write');
                        $rs = array();
