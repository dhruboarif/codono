...
class InvestController extends HomeController
{
	...
	const PROBABILITY = '49';  // winning probability index Do not keep below 10
...
/**************************INVEST CODE START **************************/		
	...
	public function listinvest()
    {
		...
		foreach($lists as $list){
			...
			$list['period']=$invest_info['period'];
	...
	public function withdraw($id){
		...
		if($investboxlog){
			 $this->show(D('Investbox')->withdraw($id));
		}else{
			$this->error('No such record');
		}
		
	}
	public function makeinvest($id=1,$amount=0)
	{
		...
		if($amount > $investbox['maxvest'] || $amount < $investbox['minvest'] ){
			    $this->error('Keep amount between minvest and maxvest');
		}
		...
		$coinnamed=strval($investbox['coinname'].'d');
		$userid=(integer)userid();
		
		$mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables codono_investbox_log write ,  codono_user_coin write  ,codono_finance write');
		$rs = array();
		...
		$begintime=time();
		$period='+'.$investbox['period'].' days';
		
		$endtime=strtotime($period, $begintime);
		
		$maturity=format_num($amount * (1+ $investbox['percentage']/100));
		...
		$insert_array=array('boxid'=>$id,'userid' => userid(),'docid'=> $docid,'amount' => $amount, 'begintime' =>$begintime, 'endtime' => $endtime,'status'=>1,'maturity'=>$maturity);
	...
	public function createinvest($coinname,$minvest=0,$period=0,$maxvest=0,$percentage=0,$coin,$balance=0,$market=0,$buytrades=0,$selltrades=0)
	{
		if (!userid()) {
            $this->error(L('PLEASE_LOGIN'));
        }
		...
		$mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables codono_investbox write');
		$rs = array();
		...
		$rs[] = $invest_insert=$mo->table('codono_investbox')->add($insert_array); 
	...
	private function ifactionFulfill($action_json,$userid){
		...
		//Buy Trades Check
		if((integer)$actionchecker->market->buy>0){
			$market=strval($actionchecker->market->name);
			$trade_count = M('TradeLog')->where(array('userid' => $userid,'status'=>1,'type'=>1,'market'=>$market))->count();
			if($trade_count>$actionchecker->market->buy){
			$res['status']=1;
			$res['message']="You have done sufficient buy trades, Required is ".$actionchecker->market->buy.$actionchecker->market->name;
			}
			else{
			$res['status']=0;
			$res['message']="You dont not sufficient buy trades [$trade_count], Required is ".$actionchecker->market->buy.' '.$actionchecker->market->name;
			return $res;
			}
		}
		
		//Sell Trades Check
		if((integer)$actionchecker->market->sell>0){
			$market=strval($actionchecker->market->name);
			$trade_count = M('TradeLog')->where(array('userid' => $userid,'status'=>1,'type'=>2,'market'=>$market))->count();
			if($trade_count>$actionchecker->market->sell){
			$res['status']=1;
			$res['message']="You have done sufficient sell trades, Required is ".$actionchecker->market->sell.$actionchecker->market->name;
			}
			else{
			$res['status']=0;
			$res['message']="You dont not sufficient sell trades [$trade_count], Required is ".$actionchecker->market->sell.' '.$actionchecker->market->name;
			return $res;
			}
		}
		return $res;
...
/**************************DICE CODE STARTS **************************/
	...
	public function highlow($call,$amount,$time){
		...
		$mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables codono_dice write ,  codono_user_coin write  ,codono_finance write');
		$rs = array();
		$userid=userid();
		$query="SELECT `$coinname`,`$coinnamed` FROM `codono_user_coin` WHERE `userid` = $userid";
		$res_bal=$mo->query($query);
		...
		$rs[] = $finance_update=$mo->table('codono_finance')->add($finance_update_array);
	...
	private function ifAllowedToDice($coinname,$amount,$time){
		if(self::DICE_ENABLE==0){die('ACCESS DENIED');}
		$balance_coin = M('UserCoin')->where(array('userid' => userid()))->getField($coinname);
    ...
    private function testme($call)
    {
		if(!$call || ($call!="high" && $call!="low")){die('select high or low');}
		$i=0;
		do {
		$i=$i+1;	
		$number=$this->probability("low");
		}while($number>50);
		echo "In this attempt it took $number attempts to get $call as current probability index is ".self::PROBABILITY;
    
    }
	private function probability($call){
