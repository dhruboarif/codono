...
class FaucetController extends HomeController
{
    public function index()
    {
        ...
        if ($tuijian) {
            ...
            if ($tuijian['zhuangtai'] == "RUNNING") {
                $tuijian['rengou'] = "<a href='/Faucet/buy/id/" . $tuijian['id'] . "'>Get Now</a>";
            }
        }
    ...
    public function buy($id = 1)
    {
        ...
        $Faucet['img'] = M('Coin')->where(array('name' => $Faucet['coinname']))->getField('img');
    ...
    public function faucetlogs($ls = 15)
    {
        if (!userid()) {
            redirect('/#login');
        }
    ...
    public function upbuy($id)
    {
		...
		*/
        ...
        $mo->execute('lock tables codono_invit write ,  codono_user_coin write  , codono_faucet write  , codono_faucet_log  write ,codono_finance write');
        $rs = array();
    ...
    public function unlock($id)
    {
        ...
        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables codono_user_coin write  , codono_faucet_log write ');
        $rs = array();
