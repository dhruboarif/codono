...
{
	const ENABLE_SUBS = ENABLE_SUBS;  // winning probability index Do not keep below 10
	...
	public function xbuy($id)
    {
		echo "trying to buy".$id;
    }
	public function buy($id = 0)
    {
        ...
        $mo->execute('lock tables codono_user write,codono_user_coin write,codono_user_subscription write,codono_mytx write,codono_finance write');
        $rs = array();
        ...
        $finance_hash = md5(userid() . $finance_num_user_coin['usd'] . $finance_num_user_coin['usdd'] . time().rand(100000,999999));
