...
class ChartController extends HomeController
{
    public function getJsonData($market = NULL, $ajax = 'json')
    {
        if ($market) {
            ...
            if (!$codono_getCoreConfig) {
                ...
                $codono_putong = $codono_getCoreConfig['codono_userTradeDetailNum'];
                $codono_teshu = $codono_getCoreConfig['codono_specialUserTradeDetailNum'];
            }
    ...
    protected function getTradeBuy($market, $limit)
    {
        ...
        if ($buy) {
            ...
            foreach ($buy as $k => $v) {
                $data .= '<tr><td class=\'buy\'  width=\'50\'>Buy' . ($k + 1) . '</td><td class=\'buy\'  width=\'80\'>' . floatval($v['price']) . '</td><td class=\'buy\'  width=\'120\'>' . floatval($v['nums']) . '</td><td  width=\'100\'><span class=\'buySpan\' style=\'width: ' . ((($maxNums < $v['nums'] ? $maxNums : $v['nums']) / $maxNums) * 100) . 'px;\' ></span></td></tr>';
            }
        }
    ...
    protected function getTradeSell($market, $limit)
    {
        ...
        if ($sell) {
            ...
            foreach ($sell as $k => $v) {
                $data .= '<tr><td class=\'sell\'  width=\'50\'>Sale' . ($k + 1) . '</td><td class=\'sell\'  width=\'80\'>' . floatval($v['price']) . '</td><td class=\'sell\'  width=\'120\'>' . floatval($v['nums']) . '</td><td style=\'width: 100px;\'><span class=\'sellSpan\' style=\'width: ' . ((($maxNums < $v['nums'] ? $maxNums : $v['nums']) / $maxNums) * 100) . 'px;\' ></span></td></tr>';
            }
        }
