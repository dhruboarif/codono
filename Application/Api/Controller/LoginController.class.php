...
class LoginController extends CommonController
{
	...
	protected function _initialize(){
		...
		/*
		$val=M('codono_verifyx')->query('select 1 from `codono_verify` LIMIT 0');
		if($val == FALSE)
		{
		//Create table codono_verify;
		$sql="CREATE TABLE `codono_verify` (
  ...
  `attempts` tinyint(2) NOT NULL DEFAULT '0' COMMENT 'number of attempts',
	PRIMARY KEY (`id`),
	UNIQUE KEY `id` (`id`)) ENGINE=MyISAM AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;";
	M('codono_verifyx')->query($sql);
		}
		*/
	}
	...
	public function testverify(){
		exit;
	//	echo $this->verification_code('wroupon@gmail.com','123456','verify');
	}
	     ...
	     if (check_arr($rs)) {
           ...
           $this->error('Code could not be added,Please try again in sometime!');
			...
			if($last_entry){
				...
				else{$this->error('Incorrect Code!');}
			}else{$this->error('Incorrect Code!');}
		}
	}
	...
	public function emailregister()
    {
        ...
        $rs = array();
        $rs[] = $mo->table('codono_user')->add(array( 'email' => $email,'username' => $username, 'addtime' => time(), 'password' => md5($password), 'invit' => $tradeno, 'tpwdsetting' => 1, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1));
    ...
    public function submit()
    {
        ...
        if (check($username, 'email')) {
            ...
            $remark = 'API Login by email:'.date('Y-m-d H:i:s');
        }

        if (!$user && check($username, 'username')) {
            $user = M('User')->where(array('username' => $username))->find();
            $remark = 'API Login by username:'.date('Y-m-d H:i:s');
        }
	...
	public function check2fa(){
        ...
        $rs = array();
        $rs[] = $mo->table('codono_user')->where(array('id' => $user['id']))->setInc('logins', 1);
        $rs[] = $mo->table('codono_user_log')->add(array('userid' => $user['id'], 'type' => 'APP log in', 'remark' => '', 'addtime' => time(), 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'status' => 1));
