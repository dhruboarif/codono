...
class ToolsController extends AdminController
{
	...
	foreach ($defined as $key=>$value){
		if($key=='THINK_VERSION'){break;}
	switch($key){
	case 'DB_PWD':
	break;
	default:
	$data[$i]=array('name'=>$key,'value'=>$value);
	}	
		$i++;
	}
    ...
    {
        //PHPTraversalfilefolder下allfile
        $handle = opendir($dir . ".");
//definitionForstoragefilenameArray 
    ...
    public function queue()
    {
        ...
        foreach ($timeArr as $key => $val) {
            if ($key == 0) {
                $val = '<br/>Last execution:' . addtime($val);
            }

            if ($key == 1) {
                $val = '<br/>Second Last execution:' . addtime($val);
            }

            if ($key == 2) {
                $val = '<br/>Third Last Execution:' . addtime($val);
            }
        ...
        if (3 <= $count) {
            ...
            if (60 < abs($timeArr[0] - time())) {
                $status = '<span class="btn btn-warning">queue stop run</span>';
            }

            if ((50 < abs($_t1)) && (50 < abs($_t2))) {
                $status = '<span class="btn">Queue operating normally</span>';
            } else {
                //$status = '<span class="btn btn-warning">Abnormal queue time,Please try again later</span>';
                $status = '<span class="btn">Queue operating normally</span>';
            }
        } else {
            $msg = '';
            ...
            if ($count == 0) {
                $msg = 'Queue has not yet started running,Refresh after sometime';
            }

            if ($count == 1) {
                $msg = 'queuerun1 Secondary please again Wait 2min checks';
            }

            if ($count == 2) {
                $msg = 'queuerun2 Secondary please again Wait 1min checks';
            }

            $status = '<span class="alert alert-warning">' . $msg . '</span>';
        }
    ...
    public function checkUpdata()
    {
        if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
            ...
            if ($list[1]) {
                ...
                M('Menu')->add(array('url' => 'Tools/index', 'title' => 'Clear the cache', 'pid' => 9, 'sort' => 1, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            } else {
                M('Menu')->where(array(
                    'url' => 'Tools/index',
                    'pid' => array('neq', 0)
                ))->save(array('title' => 'Clear the cache', 'pid' => 9, 'sort' => 1, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            }
            ...
            if ($list[1]) {
                ...
                M('Menu')->add(array('url' => 'Tools/dataExport', 'title' => 'backup database', 'pid' => 9, 'sort' => 2, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            } else {
                M('Menu')->where(array(
                    'url' => 'Tools/dataExport',
                    'pid' => array('neq', 0)
                ))->save(array('title' => 'backup database', 'pid' => 9, 'sort' => 2, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            }
            ...
            if ($list[1]) {
                ...
                M('Menu')->add(array('url' => 'Tools/dataImport', 'title' => 'Restore Database', 'pid' => 9, 'sort' => 2, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            } else {
                M('Menu')->where(array(
                    'url' => 'Tools/dataImport',
                    'pid' => array('neq', 0)
                ))->save(array('title' => 'Restore Database', 'pid' => 9, 'sort' => 2, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            }
            ...
            if ($list[1]) {
                ...
                M('Menu')->add(array('url' => 'Tools/queue', 'title' => 'Server queue', 'pid' => 9, 'sort' => 3, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            } else {
                M('Menu')->where(array(
                    'url' => 'Tools/queue',
                    'pid' => array('neq', 0)
                ))->save(array('title' => 'Server queue', 'pid' => 9, 'sort' => 3, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            }
            ...
            if ($list[1]) {
                ...
                M('Menu')->add(array('url' => 'Tools/wallet', 'title' => 'Check the wallet', 'pid' => 9, 'sort' => 3, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            } else {
                M('Menu')->where(array(
                    'url' => 'Tools/wallet',
                    'pid' => array('neq', 0)
                ))->save(array('title' => 'Check the wallet', 'pid' => 9, 'sort' => 3, 'hide' => 0, 'group' => 'tool', 'ico_name' => 'wrench'));
            }
            ...
            if (!in_array('class', $DbFields)) {
                M()->execute('ALTER TABLE `codono_version_game` ADD COLUMN `class` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
            }

            if (!in_array('shuoming', $DbFields)) {
                M()->execute('ALTER TABLE `codono_version_game` ADD COLUMN `shuoming` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
            }

            if (!in_array('gongsi', $DbFields)) {
                M()->execute('ALTER TABLE `codono_version_game` ADD COLUMN `gongsi` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `title`;');
            }
