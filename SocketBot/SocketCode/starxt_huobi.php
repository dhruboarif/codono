                ...
                switch ($pieces[2]) {
                    ...
                    case "depth" :   //Depth
                        ...
                        for ($i = 0; $i < count($bids); $i++) {  //Buy orders
                            $msg['buy'][$i]['id'] = $i;
                            $msg['buy'][$i]['price'] = $bids[$i][0];
                            $msg['buy'][$i]['quantity'] = $bids[$i][1];
                            if ($i == 0) {
                                $msg['buy'][$i]['total'] = $bids[$i][1];
                            } else {
                                $msg['buy'][$i]['total'] = $bids[$i][1] + $bids[$i - 1][1];
                            }
							$msg['buyvol']=$msg['buyvol']+ $msg['buy'][$i]['total'];
                        }

                        for ($i = 0; $i < count($asks); $i++) {  //Sell Orders
                            $msg['sell'][$i]['id'] = $i;
                            $msg['sell'][$i]['price'] = $asks[$i][0];
                            $msg['sell'][$i]['quantity'] = $asks[$i][1];
                            if ($i == 0) {
                                $msg['sell'][$i]['total'] = $asks[$i][1];
                            } else {
                                $msg['sell'][$i]['total'] = $asks[$i][1] + $asks[$i - 1][1];
                            }
							
							$msg['sellvol']=$msg['sellvol']+ $msg['sell'][$i]['total'];
                        }
                    ...
                    case "trade":     //Real-time deals
						...
						$newprice[$pieces[1]]=$data['tick']['data'][0]['price'];
